...
class TestPhasenTrennung:
    async def test_mechanik_pausiert_bis_continue(
        ...
        narrations_before = len(started_game["state"]["narrations"])
        ...
        assert state["dice_rolls"], "Die Wuerfe muessen schon feststehen"
        assert len(state["narrations"]) == narrations_before, (
            "Ohne Bestaetigung darf noch keine neue Erzaehlung entstanden sein"
        )
        turn_id = state["turn"]["id"]

        # Nicht nur die Spielleitung darf bestaetigen.
        response = await client.post(
            f"/api/v1/games/{game_id}/continue", headers=auth(guest["token"])
        )
        ...
        assert state["turn"]["status"] == "collecting"
        assert len(state["narrations"]) > narrations_before

    async def test_doppeltes_continue_wird_abgelehnt(
        self, client: AsyncClient, started_game: dict[str, Any]
    ) -> None:
        game_id = started_game["game_id"]
        host, guest = started_game["host"], started_game["guest"]
        ...
        first = await client.post(
            f"/api/v1/games/{game_id}/continue", headers=auth(host["token"])
        )
